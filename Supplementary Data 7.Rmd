...
Import dataset:
```{r}
d1 <- read_excel("./SynProt GABAAR Subunits 1.xlsx")
d2 <- read_excel("./SynProt GABAAR Subunits 2.xlsx")
...
```{r}
subset_d2_columns <- 3:8 #columns you need to sort into alphabetical order; it should be column 3 to the end
...
EDMCC <- controls %>%
  pivot_longer(cols = c(GABRA1:GABRG2), 
               names_to = "Variable", 
               values_to = "EDMCC") %>%
