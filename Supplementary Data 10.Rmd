---
title: "Synaptic Proteomic Expression Analysis of Ionotropic Receptor Subunits, Trafficking, and Scaffolding Proteins in the Human Brain - 2025 Version"
author: "Kevin Shen & Agenor Limon & Julia Granchi"
output: html_notebook
---

description: The following write up includes the R code utilized in our analysis of the expression patterns of GABA-A Receptor trafficking proteins.
...
Import dataset:
```{r}
d1 <- read_excel("./SynProt GABAAR traffic 1.xlsx")
d2 <- read_excel("./SynProt GABAAR traffic 2.xlsx")
...
Heatmap_AUD <- d1 %>% #Generate data for heatmap
  select(Protein, AUD905:AUD994) %>% #
  group_by(Protein) %>% #Run all following functions in structure groups
  summarise_all(list(mean = mean)) %>% #Calculate mean expression using list()
  data.frame()

rownames(Heatmap_AUD) <- paste0(Heatmap_AUD$Protein) #Define Y axis labels
```

Creation of chosen transcript dendrogram for control samples  
```{r}
Protein <- hclust(dist(Heatmap_AUD), method = "ward.D2") #Cluster structures using ward method like previously
par(pin = c(10, 5))
par(mar = c(5, 15, 1, 5))
dendroAUD <- as.dendrogram(Protein) %>% plot(horiz = TRUE, main = "Clustering of structures for AUD subjects")  #Visualize clustering 
```
Prepare for clustering of chosen transcripts in control samples 
```{r}
Heatmap_CON <- d1 %>% #Generate data for heatmap
  select(Protein, CON907:CON997) %>% #
  group_by(Protein) %>% #Run all following functions in structure groups
  summarise_all(list(mean = mean)) %>% #Calculate mean expression using list()
  data.frame()

rownames(Heatmap_CON) <- paste0(Heatmap_CON$Protein) #Define Y axis labels
```

Creation of chosen protein dendrogram for control samples 
```{r}
Protein <- hclust(dist(Heatmap_CON), method = "ward.D2") #Cluster structures using ward method like previously
par(pin = c(10, 5))
par(mar = c(5, 15, 1, 5))
dendroAUD <- as.dendrogram(Protein) %>% plot(horiz = TRUE, main = "Clustering of structures for control subjects")  #Visualize clustering 
```
...
rownames(Complex_Annotation) <- d1$Protein


```

Simple protein expression heat map comparing transcript levels of AUD and control samples without clustering data

```{r}
Heatmap <- d1 %>% #Generate data for heatmap
  select(Protein, AUD905:CON997) %>% #
  group_by(Protein) %>% #Run all following functions in structure groups
  summarise_all(list(mean = mean)) %>% #Calculate mean expression using list()
  data.frame()

# Set the row names to the Protein names
rownames(Heatmap) <- paste0(Heatmap$Protein)

# Sort the rows by the Protein names alphabetically
...
fontsize = 5,
main = "Synaptic Protein Expression Heatmap for AUD and control subjects"  # Title for the heatmap
)
```


Protein expression heat map comparing transcript levels of AUD and control samples with clustering data
```{r}
Heatmap <- d1 %>% #Generate data for heatmap
  select(Protein, AUD905:CON997) %>% #
  group_by(Protein) %>% #Run all following functions in structure groups
...
protein expression heat map of AUD samples with clustering data
```{r}
pheatmap(
...
annotation_col = Complex_Annotation,
fontsize = 5,
main = "Protein Expression Heatmap for AUD subjects"  # Title for the heatmap
)

```
Protein expression heat map of Control samples with clustering data
```{r}
pheatmap(
...
annotation_col = Complex_Annotation,
fontsize = 5,
main = "Synaptic Protein Expression Heatmap for CON subjects"  # Title for the heatmap
)
```
...
```{r}
subset_d2_columns <- 3:9 #columns you need to sort into alphabetical order; it should be column 3 to the end
...
EDMCC <- controls %>%
  pivot_longer(cols = c(AP2A1:PLCL2), 
               names_to = "Variable", 
               values_to = "EDMCC") %>%
...
```{r}
diagnosis <- c("AUD","AUD","AUD","AUD","AUD","AUD","AUD","AUD","AUD","AUD","CTRL","CTRL","CTRL","CTRL","CTRL","CTRL","CTRL","CTRL","CTRL")
